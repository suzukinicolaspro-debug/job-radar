...
KEYWORDS_INTL = [
    ...
    # que contrata muito por idioma (PT-BR/ES) e paga em dólar, além de ser
    # o mesmo tipo de trabalho de curadoria/avaliação de dataset já feito
    # em produção (fine-tuning de LLM), então entra como categoria própria
    # de cargo, não mistura com as de análise.
    "Data Annotator",
    "Data Annotation",
    "AI Evaluator",
    "AI Trainer",
...
TERMOS_BUSCA_INTL = [
    ...
    "data scientist spanish speaker",
    "data scientist spanish speaking",
    "data scientist portuguese speaker",
    "data scientist portuguese speaking",
    "machine learning engineer spanish speaker",
    "machine learning engineer portuguese speaker",
    "remote data analyst latam",
    "remote data analyst latin america",
    "remote data scientist latam",
    "remote data scientist latin america",
    "data analyst spanish market",
    "data scientist spanish market",
    "analista de datos remoto",
    "cientista de dados remoto",
    # Termo de cargo puro, escopado por país aceito (ver comentário logo
    # abaixo sobre por que "não faz sentido buscar só 'data analyst'
    # sozinho" não vale aqui): todo termo desta lista já roda escopado por
    # país (LOCATIONS_INTL) + remoto (f_WT=2) — nunca é busca global. E o
    # filtro de idioma pós-busca (RegrasFiltro.idiomas_exigidos) só entra
    # em jogo quando a vaga NÃO declara mercado nenhum no texto — quando o
    # local já é um país aceito (ex: Colômbia), o PAÍS é o sinal, dispensa
    # achar "spanish"/"portuguese" no título (mesma regra que já vale pro
    # resto do filtro, ver job.py).
    "data analyst",
    "data scientist",
    "machine learning engineer",
