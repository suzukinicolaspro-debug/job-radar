...
# Ordem importa: do mais específico pro mais genérico. Classificação usada
# tanto pro score (pontuar_relevancia) quanto, opcionalmente, pro filtro
# rígido (RegrasFiltro.somente_junior, ver Job.combina_com) — por padrão só
# classifica, não exclui nada; passa a excluir quando somente_junior=True.
_NIVEIS_SENIORIDADE = [
...
def _detectar_senioridade(titulo: str) -> str:
    """Classifica o nível pelo título. Por padrão isso é só informativo
    (júnior pode virar sênior lendo a descrição, e "PL"/"Sr" no título nem
    sempre reflete o que a empresa pede de verdade) — mas com
    RegrasFiltro.somente_junior=True o resultado também decide se a vaga
    passa no filtro, não só a posição no ranking.
    """
    titulo_norm = _normalizar(titulo)
...
class RegrasFiltro:
    ...
    # Filtro rígido de senioridade (diferente do score, que só reordena —
    # ver _PESO_SENIORIDADE_ALVO): quando True, só passa vaga classificada
    # como "Júnior" ou sem nível nenhum no título ("Não especificado"/
    # numeral romano solto, ver _detectar_senioridade). Barra Pleno,
    # Sênior/Especialista/Liderança e Estágio/Trainee. Decisão do usuário
    # (não é o que a maioria das vagas de dados/ciência de dados no Brasil
    # declara — a maior parte não menciona nível nenhum no título, por isso
    # "sem nível" passa junto com "Júnior" em vez de exigir a palavra
    # explícita, que reduziria demais o volume).
    somente_junior: bool = False
...
def _nivel_e_junior_ou_neutro(nivel: str) -> bool:
    """Usado só por RegrasFiltro.somente_junior (ver Job.combina_com) — nível
    "sem sinal contra" pra quem busca vaga júnior: o próprio "Júnior", ou
    título que não menciona nível nenhum ("Não especificado"/numeral romano
    solto, ver _detectar_senioridade). Deixa de fora Pleno, Sênior/
    Especialista/Liderança e Estágio/Trainee.
    """
    return nivel == "Júnior" or nivel == "Não especificado" or nivel.startswith("Nível ")
...
class Job:
    ...
    def senioridade(self) -> str:
        ...
        Por padrão é só informativo pra notificação/score — mas
        RegrasFiltro.somente_junior=True usa esse mesmo valor pra excluir
        vaga no filtro (ver combina_com).
        """
    ...
    def combina_com(self, regras: RegrasFiltro) -> bool:
        """Verifica se a vaga bate com pelo menos uma keyword E uma cidade/
        modalidade E (se regras.somente_junior) um nível aceito.
        ...
        - keywords_forte: só existe mesmo em vaga de dados/ciência de dados,
          basta bater no título.
        - keywords_ambiguo: também é usado em vaga de outra área (ex:
          ...
          "machine learning"). É o que permite ir adicionando cargo adjacente
    ...
    def _avaliar(self, regras: RegrasFiltro) -> _Avaliacao:
        ...
        bate_senioridade = not regras.somente_junior or _nivel_e_junior_ou_neutro(self.senioridade)

        return _Avaliacao(
            aprovada=bate_keyword and bate_cidade and bate_senioridade,
    ...
    def pontuar_relevancia(self, regras: RegrasFiltro) -> int:
        ...
        - Cargo no título: 3 se bateu keyword forte, 2 se bateu só o par
          ...
          nenhum no título).
        - Ferramenta no título (Machine Learning, SQL, Python...): 2.
    ...
    def motivo_aprovacao(self, regras: RegrasFiltro) -> str:
        ...
        - "Cargo forte": bateu keyword inequívoca de dados.
        ...
        - "Ferramenta + cargo": aprovou por ferramenta no título (ex:
          "Machine Learning"), não por palavra de cargo.
